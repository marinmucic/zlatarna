...
$q = 'SELECT FORMAT(total, 2) AS total, FORMAT (shipping,2) AS shipping, credit_card_number,
DATE_FORMAT(order_date, "%a %b %e, %Y at %h:%i%p") AS od, email, CONCAT(last_name, ", ", first_name) AS
name, CONCAT_WS(" ", address1, address2, city, state, zip) AS address, phone, customer_id, CONCAT_WS("
- ", cat.category, p.description) AS item, p.stock, quantity, FORMAT(price_per,2) AS price_per,
DATE_FORMAT(ship_date, "%b %e, %Y") AS sd FROM orders AS o INNER JOIN customers AS c ON (o.customer_id
= c.id) INNER JOIN order_contents AS oc ON (oc.order_id = o.id) INNER JOIN products AS p
ON (oc.product_id = p.id ) INNER JOIN categories AS cat ON
(cat.id = p.categories_id) WHERE o.id=' . $order_id;
...
if (mysqli_num_rows($r) > 0) {
    ...
    echo '<p><strong>Order ID</strong>: ' . $order_id . '<br />
        ...
        '<br /><strong>Shipping</strong>: $' . $row['shipping'] . '<br />
    ...
    if (!$shipped) {
        echo '<div class="field"><p class="error">Note that actual payments will be collected once you click
        this button!</p><input type="submit" value="Ship This Order" class="button" /></div>';
    }
...
$q = "SELECT customer_id, total, transaction_id FROM orders AS o JOIN transactions AS t ON
(o.id=t.order_id AND t.type='auth_only' AND t.response_code=1) WHERE o.id=$order_id";
$r = mysqli_query($dbc, $q);
    ...
    if (mysqli_num_rows($r) === 1) {
        ...
        if ($order_total > 0) {
            ...
            require('../includes/sdk/AuthorizeNet.php');
            ...
            $r = mysqli_query($dbc, "CALL add_transaction($order_id, '{$response->transaction_type}', $order_total,
            {$response->response_code}, '$reason', {$response->transaction_id}, '$full_response')");
            ...
            if ($response->approved) {
                ...
                $q = 'UPDATE specific_coffees AS sc, order_contents AS oc SET sc.stock=sc.stock-oc.quantity WHERE
                sc.id=oc.product_id AND oc.product_type="coffee" AND oc.order_id=' . $order_id;
                $r = mysqli_query($dbc, $q);
                $q = 'UPDATE non_coffee_products AS ncp, order_contents AS oc SET ncp.stock=ncp.stock-oc.quantity WHERE
                ncp.id=oc.product_id AND oc.product_type="goodies" AND oc.order_id=' . $order_id;
                $r = mysqli_query($dbc, $q);
